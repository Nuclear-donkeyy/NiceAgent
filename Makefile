...
.PHONY: run-control run-runtime run-sandbox run-web build-web test smoke-three-services smoke-three-services-ui smoke-three-services-redis smoke-control-plane-fanout smoke-sse-replay smoke-artifact-cleanup smoke-redis-capacity smoke-sandbox-container smoke-deepseek-runtime compose-up compose-down compose-config check-js check-scripts check-alerts check-k8s-sandbox docker-build docker-build-control docker-build-runtime docker-build-sandbox kind-create kind-delete kind-load k8s-apply k8s-status k8s-port-forward kind-deploy
