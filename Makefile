...
.PHONY: run-control run-runtime run-sandbox run-web build-web test smoke-three-services smoke-three-services-ui smoke-three-services-redis smoke-control-plane-fanout smoke-sse-replay smoke-http-skill-backend smoke-artifact-cleanup smoke-redis-capacity smoke-sandbox-container smoke-deepseek-runtime compose-up compose-down compose-config check-js check-scripts check-alerts check-k8s-sandbox docker-build docker-build-control docker-build-runtime docker-build-sandbox kind-create kind-delete kind-load k8s-apply k8s-status k8s-port-forward kind-deploy
...
smoke-sse-replay:
	python3 scripts/smoke_sse_replay.py

smoke-http-skill-backend:
	python3 scripts/smoke_http_skill_backend.py

smoke-artifact-cleanup:
	python3 scripts/smoke_artifact_cleanup.py

smoke-redis-capacity:
	python3 scripts/smoke_redis_capacity.py --start-redis
