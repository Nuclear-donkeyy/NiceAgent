...
用户发起一次聊天请求后，系统会创建一个可追踪的 `Run`。`Run` 会被前置服务器调度到某个 agent 实例执行。前端以聊天消息为主，实时折叠展示 agent 当前状态，例如模型生成、调用 skill、通过系统 CLI 获取外部信息、失败原因和 artifacts；底层 `RunEvent` 主要作为内部审计、replay 和排障协议。
...
6. 执行过程产生标准化 `RunEvent`，前端通过 SSE/WebSocket 折叠为 assistant 回复和 agent 状态。
...
CLI 是 Agent Runtime 的系统级工具，用来获取外部世界信息，不需要用户逐次授权；但它必须通过 Sandbox Executor 执行，并受只读网络型策略、资源限制、超时和审计约束。
