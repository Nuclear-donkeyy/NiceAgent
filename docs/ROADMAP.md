...
- Control Plane 仍缺 NiceAgent 内置 OIDC/session/JWT、强一致账单级 quota、真实值班系统接入和容量看板；邀请创建/接受、可信身份绑定、可选 SMTP 邮件、投递模板、进程内内存队列、durable outbox、重试和 provider-neutral 退信/投诉/丢弃事件记录已有最小闭环；Redis Runtime worker、重试/lease、跨副本 event fanout、审计、结构化 request log、基础 metrics/tracing、Prometheus 告警规则、Alertmanager 路由样例、Redis 低层命令 spans 和 Postgres repository spans 已有最小闭环。
