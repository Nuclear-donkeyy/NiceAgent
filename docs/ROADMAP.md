...
- Control Plane 仍缺 NiceAgent 内置 OIDC/session/JWT、邮件发送、强一致账单级 quota 和外部告警；Redis Runtime worker、重试/lease、跨副本 event fanout、审计、结构化 request log、基础 metrics/tracing、Redis 低层命令 spans 和 Postgres repository spans 已有最小闭环。
...
- 在已有 audit、metrics、trace/request id、run replay、结构化 request log、Redis 低层命令 spans 和 Postgres repository spans 基础上补外部告警和基础管理排障视图。
