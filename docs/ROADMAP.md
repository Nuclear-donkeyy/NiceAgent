...
- 前端已经隐藏原始事件面板，并支持 artifact 展示、下载、图片/PDF/音频/视频预览以及 CSV/TSV 表格内容预览；`make smoke-http-skill-backend` 已覆盖 fake model tool calling 到真实 HTTPS Skill backend 的后端流；skill 配置校验、浏览器级断线恢复、端到端测试和错误恢复体验还需要继续补强。
