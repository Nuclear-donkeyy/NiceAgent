...
- Control Plane 仍缺 NiceAgent 内置 OIDC/session/JWT、邮件发送、强一致账单级 quota、日志关联和外部告警；Redis Runtime worker、重试/lease、跨副本 event fanout、审计、基础 metrics/tracing、Redis 低层命令 spans 和 Postgres repository spans 已有最小闭环。
...
- 在已有 audit、metrics、trace id、run replay、Redis 低层命令 spans 和 Postgres repository spans 基础上补日志关联、外部告警和基础管理排障视图。
