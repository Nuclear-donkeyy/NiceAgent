...
- Control Plane 仍缺完整认证、多用户/项目权限、配额、Redis run queue、跨副本 event fanout、审计日志、metrics 和 tracing。
...
- 将 Redis Streams 从占位边界推进到 run queue 和多 Control Plane event fanout 的可运行路径。
