...
- HTTP Skill SSRF 防护已覆盖静态 URL 校验和 DNS 解析后的私网地址拦截；per-skill retry、Runtime 进程内 rate limit、可选 Redis 跨副本 rate limit、Runtime 默认风险门禁、项目级风险策略配置和前端最小容量看板已有闭环，后续继续补策略告警联动、容量建议和更细审计。
