...
- Control Plane 仍缺强一致账单级 quota、真实值班系统接入和生产级容量看板；前端已有基于项目 quota policy 与 24h usage 的最小容量视图、查询/解除邀请邮件停发的“邮件治理”管理面板，以及 OIDC 登录/刷新/退出的最小会话入口。`AUTH_MODE=oidc` 已支持 bearer JWT/JWKS 资源服务器校验，也具备最小浏览器 OIDC authorization code flow、HttpOnly session cookie、refresh token 刷新、logout 和 refresh/logout CSRF 防护。邀请创建/接受、可信身份绑定、可选 SMTP 邮件、投递模板、进程内内存队列、durable outbox、重试、邀请邮件重发 API、provider-neutral 退信/投诉/丢弃事件记录、HMAC webhook 入口、SendGrid/SES/Mailgun 最小原生字段映射、组织级自动停发和 suppression 查询/解除 API 已有最小闭环；Redis Runtime worker、重试/lease、跨副本 event fanout、consumer group lag/pending/DLQ 指标、审计、结构化 request log、基础 metrics/tracing、Prometheus 告警规则、Alertmanager 路由样例、Redis 低层命令 spans 和 Postgres repository spans 已有最小闭环。
