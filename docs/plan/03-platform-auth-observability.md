...
- 邀请邮件 subject/body 模板、进程内内存队列、durable outbox、投递重试、邀请邮件重发 API、provider-neutral delivery/bounce/complaint/drop 事件记录、HMAC webhook 入口和组织级自动停发已落地；更细 action-level policy、服务商原生字段映射、suppression 解除/管理 UI 和管理后台 UI 仍待补。
