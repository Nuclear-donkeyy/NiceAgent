...
- 邀请邮件 subject/body 模板、进程内内存队列、durable outbox、投递重试、邀请邮件重发 API、provider-neutral delivery/bounce/complaint/drop 事件记录、HMAC webhook 入口、组织级自动停发、suppression 查询/解除 API 和前端“邮件治理”管理面板已落地；更细 action-level policy、服务商原生字段映射和管理后台 UI 仍待补。
