...
3. 下一条真正的平台化关键路径是 NiceAgent 内置 OIDC/session/JWT、邮件发送、真实 tokenizer/按模型动态估算与强一致账单级 quota，以及 DB/Redis 低层 OTel spans、日志关联和告警。
