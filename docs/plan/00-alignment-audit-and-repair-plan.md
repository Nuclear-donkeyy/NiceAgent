...
| --- | --- | --- | --- |
| Skill 执行生产化 | 部分对齐 | HTTP Skill schema/runtime_config 校验、Runtime 入参/出参校验、SSRF 基础拦截、DNS 解析后私网地址拦截、SecretResolver 接口、本地 secret/redaction、`env://` 和受限 `file://` secret_ref、前端 Secret Ref 输入、结构化 observation、per-skill retry、Runtime 进程内 rate limit、可选 Redis 跨副本 skill rate limit、OpenAPI JSON/YAML preview dry-run 和选中 operation 保存为 HTTP Skill 的最小导入向导；MCP `tools/list` 风格 manifest preview dry-run API 与前端面板已能生成候选 skill manifest；Redis queue worker 可通过 execution context materialize 完整 `RuntimeSkill`；`tool.started`/`tool.finished` 会写入脱敏后的 `audit_events` skill invocation 轨迹 | KMS/Vault/External Secrets 原生 backend 未接；MCP 保存/执行 adapter 未做；独立 skill invocation 专表和更细风险策略未做 |
...
- Control Plane skill API 与 tool invocation audit：`services/control-plane/internal/httpapi/server.go`
