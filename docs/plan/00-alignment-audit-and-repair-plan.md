...
3. 下一条真正的平台化关键路径是 NiceAgent 内置 OIDC/session/JWT、邮件发送、真实 tokenizer/按模型动态估算与强一致账单级 quota，以及告警路由/值班系统。
