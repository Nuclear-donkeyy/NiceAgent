...
| 模型运营与 DeepSeek | 部分对齐 | Eino 原生 `ToolCallingChatModel`、OpenAI-compatible provider、`MODEL_PROVIDER_PROFILE=deepseek` 配置预设、fake DeepSeek/OpenAI-compatible tool calling 回归测试、错误分类、retry transport、Runtime 进程内模型请求限流和并发保护、单一后备 provider fallback、usage tracker、redactor、run_usage 持久化、配置化 cost/pricing、provider health 快照、模型运行指标、可选主动 provider 探针、基础告警指标和 Alertmanager 路由样例；provider 缺失 usage 时会标记 `estimated=true` 和 `token_estimator` | 未完成真实 DeepSeek API 冒烟；复杂多 provider 路由未做；真实值班系统接入未做；跨 Runtime/provider 账号级容量协调未做；真实 usage 依赖 provider callback，缺失时仍估算 |
...
3. 邀请邮件 subject/body 模板、进程内内存队列、durable outbox 和 provider-neutral 退信事件记录已落地；下一步补服务商 webhook 签名校验、重发管理和停发策略，让邀请流程从最小闭环走向可运营。
