...
| --- | --- | --- | --- |
| Skill 执行生产化 | 部分对齐 | HTTP Skill schema/runtime_config 校验、Runtime 入参/出参校验、SSRF 基础拦截、DNS 解析后私网地址拦截、SecretResolver 接口、本地 secret/redaction、`env://` 和受限 `file://` secret_ref、前端 Secret Ref 输入、结构化 observation、per-skill retry、Runtime 进程内 rate limit、可选 Redis 跨副本 skill rate limit、Runtime 级 `SKILL_RISK_POLICY` 风险门禁、OpenAPI JSON/YAML preview dry-run 和选中 operation 保存为 HTTP Skill 的最小导入向导；MCP `tools/list` 风格 manifest preview、前端导入、保存为用户 MCP Skill 和 Runtime 最小 HTTP JSON-RPC `tools/call` adapter 已有闭环；Redis queue worker 可通过 execution context materialize 完整 `RuntimeSkill`；`tool.started`/`tool.finished` 会写入脱敏后的 `audit_events` 和 `skill_invocations` 轨迹 | KMS/Vault/External Secrets 原生 backend 未接；完整 MCP stdio/SSE transport、initialize/session negotiation 和动态 tool list invalidation 未做；项目级风险策略、UI 展示和风险策略告警未做 |
