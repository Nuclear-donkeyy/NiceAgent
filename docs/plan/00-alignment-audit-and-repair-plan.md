...
- 架构能力缺口：Redis queue 已补上最小 Runtime consumer 闭环，attempt/lease/fencing 也已有权威字段、回写校验、heartbeat 续租、idle pending auto-claim 和 DLQ；`workspace.read` 已能列出和读取文本 artifact；三服务已具备轻量 `X-Trace-ID` 传播、`/metrics` 指标、OpenTelemetry OTLP HTTP exporter、入站 HTTP span、Control Plane 调度/回写、Runtime run/tool/model、HTTP Skill、Sandbox HTTP/exec 和 Redis queue 处理 span 边界；quota 已有项目 policy、Redis 并发/小时预占、模型 token 预扣/结算和 tool/sandbox 最小实时预占；但 OIDC 还只是 header 边界，真实 tokenizer、账单维度 quota、DB/Redis 低层命令级 spans、真实 DeepSeek 冒烟、复杂模型路由仍待补齐。
...
| 前端产品体验与 E2E | 高度对齐但仍有小偏差 | React/TS/SCSS Modules、聊天优先、artifact 展示/下载、HTTP Skill 表单校验、保存状态、Playwright smoke、SSE replay 去重；HTTP Skill URL 已同步为仅允许 `https`；`make smoke-three-services` 可启动三服务真实进程做 `/cli echo hello` 冒烟 | Playwright 仍是 mock backend smoke；Redis dispatcher 冒烟需要外部 Redis；重连恢复状态还偏轻 |
...
- 增加最小 quota：`concurrent_runs`、`runs_per_hour`、`model_tokens_per_day`、`tool_calls_per_day`、`sandbox_seconds_per_day` 已完成；项目级持久配置模型已完成最小闭环；Redis 并发/小时窗口预占、固定/动态模型 token 预扣/结算、run 级 tool/sandbox/artifact 用量记录和 Runtime tool/sandbox 最小实时预占已完成。真实 tokenizer/按模型动态估算和账单维度聚合仍待补齐。
- 三服务已接入轻量 `X-Trace-ID`、标准 `traceparent`、`/metrics`、OpenTelemetry OTLP HTTP exporter、入站 HTTP server span、调度/回写、run/tool/model、HTTP Skill、Sandbox 和 Redis queue span；后续仍需补 DB/Redis 低层命令级 spans、日志关联和外部告警系统。

验收：

- 不同用户/项目不能互相读 chat/run/artifact/skill/audit。
- quota deny 可在前端显示中文错误。
- 用 `run_id` 能串起 request log、audit event；服务间调用能透传 `X-Trace-ID` 和 `traceparent`；配置 `OTEL_TRACES_EXPORTER=otlp` 后至少能看到三服务入站 HTTP、调度/回写、run/tool/model、HTTP Skill、Sandbox 和 Redis queue spans。DB/Redis 低层命令级 spans 仍是后续验收项。
...
- Playwright 增加三服务集成 smoke：启动 Control Plane、Agent Runtime、Sandbox Executor，覆盖 `/cli echo hello`、artifact list、SSE replay。
- 已新增非 Playwright 的三服务进程 smoke，覆盖 `/cli echo hello`；Playwright 三服务 UI smoke、artifact list 和 SSE replay 仍待补。
- UI 增加更明确的重连/恢复状态。
...
- usage/cost 可按 run 查询，估算值明确标记 `estimated=true`。
...
3. 下一条真正的平台化关键路径是 NiceAgent 内置 OIDC/session/JWT、邮件发送、真实 tokenizer/按模型动态估算与账单维度 quota，以及 DB/Redis 低层 OTel spans、日志关联和告警；同时跨 Control Plane event fanout 仍需要补强。
