...
| --- | --- | --- | --- |
| Skill 执行生产化 | 部分对齐 | HTTP Skill schema/runtime_config 校验、Runtime 入参/出参校验、SSRF 基础拦截、DNS 解析后私网地址拦截、SecretResolver 接口、本地 secret/redaction、`env://` secret_ref、结构化 observation、per-skill retry、Runtime 进程内 rate limit、OpenAPI JSON preview dry-run；Redis queue worker 可通过 execution context materialize 完整 `RuntimeSkill` | KMS/Vault/External Secrets 原生 backend 未接；OpenAPI 完整保存向导、YAML 支持和 MCP 导入未做；跨副本强一致 skill rate limit 和更细审计未做 |
