...
| Sandbox 与 Artifact | 中度对齐 | workspace 登记、artifact 表/API/download、前端 artifact 展示、图片/PDF/音频/视频内联预览、CSV/TSV 表格内容预览、sandbox 输出扫描、path/symlink 越界检查、container executor 可配置；Runtime 在 sandbox tool 调用后可通过内部 API 增量登记 artifact 并触发 `artifact.created` event；artifact 已支持 `expires_at`、`ARTIFACT_RETENTION_DAYS`、内部过期 metadata 清理接口和本地 workspace 文件回收；`workspace.read` 可列出当前 run artifacts、读取已登记文本 artifact 摘要，并返回 workspace/artifact 元数据摘要；K8s 已有基础 `NetworkPolicy` ingress/egress、`ResourceQuota`、`LimitRange` 和 Sandbox Executor `securityContext` | Compose 默认仍是 local executor；artifact 对象存储归档/生命周期、K8s RuntimeClass、独立节点池、云侧出口控制和强隔离方案未落地 |
...
仍待修复：artifact 对象存储归档/生命周期和更强隔离部署还没有完成；过期 metadata 清理和本地文件回收已有最小接口，图片、PDF、音频、视频和 CSV/TSV artifact 已有最小预览。
...
5. Sandbox container 默认执行路径、云侧出口控制和更完整容量建议；图片/PDF/音视频内联预览、CSV/TSV 表格预览、K8s egress NetworkPolicy、镜像白名单与 healthz 可观测已落地。
