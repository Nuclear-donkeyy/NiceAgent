...
仍待修复：artifact 对象存储归档/生命周期和更强隔离部署还没有完成；过期 metadata 清理和本地文件回收已有最小接口，图片、PDF、音频、视频和 CSV/TSV artifact 已有最小预览；容器执行路径已有 `make smoke-sandbox-container` 作为 opt-in 验收入口，但 Compose/生产默认仍需在目标运行环境具备 Docker/容器执行依赖后再切换；K8s RuntimeClass/专用节点池已有可选模板和结构检查，但生产集群仍需要安装对应 runtime handler 并显式启用。
