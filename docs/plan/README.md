...
4. 平台化收口：在 `AUTH_MODE=trusted-header|oidc`、ActorContext、RBAC、邀请、quota、metrics/tracing、告警规则、Alertmanager 路由样例、durable 邀请邮件 outbox、邀请邮件重发 API、provider-neutral 退信事件记录、HMAC webhook 入口和组织级自动停发已有最小闭环基础上，补浏览器 OIDC login/session/refresh token、更完整 tokenizer 覆盖、服务商原生字段映射、suppression 解除/管理 UI 和真实值班系统接入。
