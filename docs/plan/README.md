...
4. 平台化收口：在 `AUTH_MODE=trusted-header|oidc`、ActorContext、RBAC、邀请、quota、metrics/tracing、告警规则、Alertmanager 路由样例和 durable 邀请邮件 outbox 已有最小闭环基础上，补浏览器 OIDC login/session/refresh token、真实 tokenizer、退信处理和真实值班系统接入。
