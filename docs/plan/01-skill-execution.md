...
- Runtime 可通过 `SKILL_RISK_POLICY` 阻止 high risk、destructive 或非 read-only skill 执行，策略拒绝不会调用真实 tool，也不会预占 tool quota。
...
- 风险策略的项目级配置、UI 展示、容量/告警和 Redis rate limit 容量/告警。
...
4. 治理能力：Runtime 进程内和 Redis 跨副本 per-skill rate limit 已有最小闭环；`SKILL_RISK_POLICY` 已提供 Runtime 级风险门禁；`audit_events` 和 `skill_invocations` 已记录脱敏 skill invocation 起止轨迹；后续继续补项目级风险策略、UI 展示和更完整 metrics/tracing。
