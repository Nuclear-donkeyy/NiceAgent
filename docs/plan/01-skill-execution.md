...
- Runtime 可通过 `SKILL_RISK_POLICY` 阻止 high risk、destructive 或非 read-only skill 执行；Control Plane 可按项目配置 `skill_risk_policy` 并在前端展示/调整，策略拒绝不会调用真实 tool，也不会预占 tool quota。
...
- 真实 Grafana 导入、值班系统接入和更细审计；Runtime rate limit denial metric、Prometheus 告警样例、Alertmanager 路由、容量/排障 runbook、dashboard JSON 样例和 `make check-alerts` CI 门禁已有最小版本。
...
4. 治理能力：Runtime 进程内和 Redis 跨副本 per-skill rate limit 已有最小闭环；rate limit 命中会进入 `niceagent_skill_rate_limit_denials_total`，已有 Prometheus 告警样例、Alertmanager 路由、容量/排障 runbook 和 Grafana dashboard 样例，且 `make check-alerts` 已进入 CI 默认门禁；`SKILL_RISK_POLICY` 已提供 Runtime 默认风险门禁；项目级 `skill_risk_policy` 持久化、API、下发和前端展示已有最小闭环；风险策略命中会进入 `niceagent_skill_policy_denials_total`，并已有 Prometheus 告警样例；`audit_events` 和 `skill_invocations` 已记录脱敏 skill invocation 起止轨迹；后续继续补真实 Grafana 导入、值班系统接入和更完整 metrics/tracing。
