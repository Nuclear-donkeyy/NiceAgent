...
- Runtime `ToolBridge` materialize 授权后的 `RuntimeSkill`，支持 builtin、HTTP Skill、secret redaction、`env://` / `file://` secret_ref、入参/出参 schema validation、结构化 observation、SSRF 基础拦截、retry 和进程内 per-skill rate limit。
