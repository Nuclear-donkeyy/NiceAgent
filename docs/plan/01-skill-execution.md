...
- MCP tools/list manifest preview dry-run API 已能生成候选 skill manifest，并保留 MCP annotations。
...
- 增加 OpenAPI/MCP importer：OpenAPI JSON/YAML preview/dry-run 与选中 operation 保存已落地；MCP manifest preview/dry-run 已落地；下一步补 MCP 保存、runtime adapter 和更复杂的 secret 绑定 UI。
...
- MCP tool -> NiceAgent skill manifest preview 已能保留 MCP annotations；保存为可执行 skill 需要后续 MCP client/runtime adapter。
...
3. 导入能力：OpenAPI JSON/YAML dry-run API 与最小保存向导已落地；MCP manifest preview/dry-run 已落地；继续实现 MCP 保存、执行 adapter 和更完整前端导入体验。
...
- MCP preview 不创建 skill、不保存 secret，只返回候选 skill manifest；MCP annotations 只作为提示，不作为安全边界。
