...
4. 治理能力：Runtime 进程内和 Redis 跨副本 per-skill rate limit 已有最小闭环；后续继续补 skill invocation 审计、风险策略和更完整 metrics/tracing。
