...
Sandbox 执行后会扫描 workspace `output/` 下的新增或修改文件，生成 artifact metadata 和 workspace diff。Agent Runtime 会从 tool observation 中提取 artifacts，并在 tool 调用结束后通过 `POST /internal/runs/{run_id}/artifacts` 交给 Control Plane 增量持久化；Control Plane 会写入 `artifact.created` event。最终 run complete 仍可携带同一批 artifacts，Control Plane 会按 artifact id 幂等处理，不重复发事件。前端已有 artifact domain/API、`ArtifactList` 展示和下载入口，刷新后可按 run 恢复 artifact metadata。Control Plane 也已支持可选 `ARTIFACT_RETENTION_DAYS`、`expires_at` 和内部过期 metadata 清理接口。
...
- K8s 已有基础 `NetworkPolicy` ingress/egress、`ResourceQuota`、`LimitRange` 和 Sandbox Executor `securityContext`。
...
- Runtime 在 tool 调用后根据 Sandbox result 写入 artifacts，并触发 `artifact.created`。
- `workspace.read` 继续扩展更多 workspace 元数据和 artifact preview，但仍只能读取已登记 artifact 或经过 Control Plane 校验的只读资源。
- K8s 已有基础 NetworkPolicy ingress/egress、ResourceQuota、LimitRange 和 securityContext；容器镜像白名单已进入配置面，后续补 RuntimeClass、独立节点池、云侧出口控制和生产镜像清单。
...
- Sandbox Executor 已能扫描 output 目录并生成 artifact metadata；Runtime 已能在 sandbox tool 调用后增量登记 artifacts；前端已支持图片、PDF、音频、视频和 CSV/TSV 表格预览；artifact 过期 metadata 清理已有最小闭环；后续补更多文件类型预览、真实文件回收和对象存储归档。
...
4. K8s 加固：基础 NetworkPolicy ingress/egress、ResourceQuota、LimitRange 和 Sandbox Executor securityContext 已落地；容器镜像白名单已进入配置面，后续继续补 RuntimeClass、独立节点池、云侧出口控制和生产镜像清单。
