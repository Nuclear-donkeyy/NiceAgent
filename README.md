...
- Skill 元数据采用 Postgres/memory 双实现，按用户/项目 grant 加载；HTTP/MCP Skill 的 bearer token 不返回前端，当前支持明文开发密钥、`env://` 和受限 `file://` secret ref，并可用 Redis 做跨 Runtime 的 per-skill rate limit；生产级 KMS/Vault/External Secret 原生 resolver 后续补齐。
