...
- Sandbox 当前提供独立 Sandbox Executor 服务、local executor 和 container executor 入口；高风险 CLI 会进入等待授权状态，但尚不支持审批后恢复执行。
- Frontend 使用 React + Rspack，支持会话搜索、归档/恢复和最近 run replay 展示，风格为黑、白、微黄色，面性+线性，少圆角。
