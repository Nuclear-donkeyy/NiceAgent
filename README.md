...
- Artifact 支持 workspace metadata、列表、下载、内容预览、可选 `expires_at` 和内部过期 metadata 清理；对象存储归档和真实文件回收后续继续补齐。
...
- Skill 元数据采用 Postgres/memory 双实现，按用户/项目 grant 加载；HTTP Skill 的 bearer token 不返回前端，当前支持明文开发密钥、`env://` 和受限 `file://` secret ref，并可用 Redis 做跨 Runtime 的 per-skill rate limit；生产级 KMS/Vault/External Secret 原生 resolver 后续补齐。
