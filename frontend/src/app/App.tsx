...
import conversationStyles from "../features/conversation/Conversation.module.css";
      ...
      <section className={styles.main}>
        <header className={styles.topbar}>
          <div>
            ...
            {workspace.activeChat?.archived && <p className={styles.topbarNote}>此会话已归档，可恢复后继续使用。</p>}
          </div>
          <div className={styles.runState}>
            <span className={[styles.status, styles[workspace.runStatus] || ""].filter(Boolean).join(" ")}>
              {statusText[workspace.runStatus] || workspace.runStatus}
            </span>
            {workspace.activeChat && (
              <button
                className={styles.lineButton}
                onClick={() => void workspace.setActiveChatArchived(!workspace.activeChat?.archived)}
                type="button"
              >
                {workspace.activeChat.archived ? "恢复" : "归档"}
              </button>
            )}
            <button className={styles.lineButton} disabled={!workspace.canCancel} onClick={() => void workspace.cancelRun()} type="button">
              取消
            </button>
          </div>
        </header>
